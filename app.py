...
def load_backend_records():
    ...
    # Inbox is an iterable object, so checking ``if not inbox`` does not
    # reliably tell us whether it contains any emails. Materialise the
    # iterable once and use the actual email count for the empty-inbox
    # fallback. This guarantees the dashboard always receives the expected
    # schema, including the Status column.
    try:
        emails = list(inbox)
...
with st.sidebar:
    ...
    with refresh_col:
        if st.button("Refresh data", use_container_width=True, key="sidebar_refresh"):
            ...
            st.session_state["comparison_df"] = load_backend_comparison()
            st.session_state["activity_log"] = []
            st.toast("Workspace data refreshed.", icon="↻")
            st.rerun()
    with version_col:
        st.markdown(
